...
<head>
    ...
    <title>Registrasi Pemain - {{ config('app.name', 'Logic Labyrinth') }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <style>
        /* bintik halus sesuai tema */
        ...
        .nav-shell.scrolled { background-color: #ffffff; box-shadow: 0 10px 26px rgba(0,0,0,0.07); border-color: rgba(229,231,235,.9); padding: .75rem 26px; transform: scale(1.01); }
...
<body class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-orange-50 flex flex-col relative overflow-x-hidden bg-ll text-gray-900">
    ...
    <main class="max-w-6xl mx-auto px-6 md:px-8 pb-16 w-full flex-1">
        <div class="grid grid-cols-1 md:[grid-template-columns:1.1fr_.9fr] gap-10 items-center mt-20">

            <!-- Form Card -->
            <div class="rounded-3xl border border-gray-200/80 bg-white/80 backdrop-blur-sm shadow-[0_10px_30px_rgba(79,70,229,0.08)] p-6 md:p-8">
                <div class="mb-6">
                    <h1 class="mt-2 text-[1.8rem] md:text-4xl font-extrabold leading-tight">
                        <span class="text-blue-800">Registrasi </span><span class="text-orange-500">Pemain</span>
                    </h1>
                    <p class="text-gray-600 mt-2">Isi data singkat di bawah ini untuk mulai bermain dan memanjat leaderboard.</p>
                </div>

                @if ($errors->any())
                    <div class="mb-4 p-3 rounded-xl bg-red-50 text-red-700 ring-1 ring-red-200">
                ...
                <form method="POST" action="{{ route('player.register.store') }}" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-1">Nama</label>
                        <input type="text" name="name" value="{{ old('name') }}" maxlength="100" required
                               class="w-full rounded-xl bg-white border border-gray-200 px-4 py-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"/>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-1">Universitas</label>
                        <input type="text" name="university" value="{{ old('university') }}" maxlength="150" required
                               class="w-full rounded-xl bg-white border border-gray-200 px-4 py-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"/>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-1">Prodi</label>
                        <input type="text" name="program" value="{{ old('program') }}" maxlength="100" required
                               class="w-full rounded-xl bg-white border border-gray-200 px-4 py-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"/>
                    </div>

                    <div class="flex items-center gap-3 pt-2">
                        <button type="submit" class="btn-ll inline-flex items-center justify-center px-5 py-2.5 rounded-lg bg-blue-700 text-white font-bold hover:bg-blue-800 hover:scale-105 hover:shadow-2xl transition-all">Mulai</button>
                        <a href="/" class="btn-ll inline-flex items-center justify-center px-4 py-2 rounded-lg bg-white text-blue-700 font-semibold ring-1 ring-blue-200 hover:bg-blue-50 transition">Kembali</a>
                    </div>
                </form>
            </div>
            ...
            <div class="hidden md:flex items-center justify-center">
                <div class="rounded-[22px] p-[1.6px] bg-gradient-to-b from-indigo-400/70 via-fuchsia-400/60 to-amber-300/70 shadow-[0_10px_30px_rgba(79,70,229,0.14)]">
                    <div class="rounded-[20px] bg-white/80 p-4">
                        <img src="/images/icons/form.png" alt="Registrasi Ilustrasi" class="w-[380px] h-auto object-contain"/>
                    </div>
                </div>
            </div>
        </div>
